...
final class TeamOver05Analyser
{
    public function __construct(
        ...
        private MarketStatusResolver $status,
    ) {}

    public function analyse(string $teamName, TeamGoalEvidence $evidence): MarketAnalysisResult
    {
        $dataQuality = $this->quality->score($evidence->sampleSize, $evidence->highVarianceCompetition);

        $score = 20
            + ($evidence->teamScoredRate * 22)
            + ($evidence->venueScoredRate * 18)
            + ($evidence->opponentConcededRate * 17)
            + ($evidence->opponentVenueConcededRate * 13)
            - ($evidence->opponentCleanSheetRate * 12)
            - ($evidence->teamFailedToScoreRate * 12)
            - ($evidence->majorAttackerMissing ? 7 : 0)
            - ($evidence->rotationRisk ? 6 : 0)
            - ($evidence->highVarianceCompetition ? 8 : 0);
        ...
        return new MarketAnalysisResult(
            MarketType::TEAM_OVER_05,
            $teamName.' Over 0.5 Goals',
            $score,
            $dataQuality,
            $this->status->resolve($score, $dataQuality),
            $positive,
            $contradictions,
            $contradictions[0] ?? 'Normal football variance.',
        );
    }
}
